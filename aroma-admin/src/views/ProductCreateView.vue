  ...
  <div class="min-h-full animate-fade-up">
    ...
    <div class="sticky top-0 z-10 bg-dash-bg border-b border-dash-border -mx-6 px-6 py-3 mb-6 flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="text-2xs text-dash-muted leading-none mb-0.5">{{ t('productCreate.productsNav') }}</p>
          <h1 class="text-sm font-semibold text-dash-text leading-none">{{ t('productCreate.newProduct') }}</h1>
        </div>
      </div>
      ...
      <div class="space-y-4">
        ...
        <section class="bg-dash-surface rounded-card shadow-card p-5">
          ...
          <ATextarea
            ...
            :placeholder="t('productCreate.descriptionHint')"
            class="!rows-4"
            rows="4"
          />
        </section>
        ...
        <section class="bg-dash-surface rounded-card shadow-card p-5">
          ...
          <div class="space-y-3">
            <ASelect
              ...
              :placeholder="t('productCreate.chooseBrand')"
              :error="formErrors.brand_id"
            />
            <ASelect
              v-model="form.category_id"
              :label="t('productCreate.categoryLabel')"
              :options="categoryOptions"
              :placeholder="t('productCreate.chooseCategory')"
...
async function handleSave() {
  formErrors.value = {}
  if (!form.value.name)        { formErrors.value.name     = t('productCreate.arabicNameRequired'); return }
  if (!form.value.slug)        { formErrors.value.name_en  = t('productCreate.englishNameRequired'); return }
  if (!form.value.brand_id)    { formErrors.value.brand_id    = t('productCreate.selectBrandRequired'); return }
  if (!form.value.category_id) { formErrors.value.category_id = t('productCreate.selectCategoryRequired'); return }
  if (!form.value.type)        { formErrors.value.type        = t('productCreate.selectTypeRequired'); return }
